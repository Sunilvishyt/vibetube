...
export default function LikeButton({ videoId }) {
  ...
  return (
    <button
      onClick={toggleLike}
      disabled={loading}
      className={`px-3 py-2 rounded-lg border cursor-pointer flex items-center justify-center space-x-1 font-semibold transition-colors duration-200 ${buttonClasses} disabled:opacity-50 disabled:cursor-not-allowed`}
    >
      {loading ? (
        <Loader2 size={17} className="animate-spin" />
      ) : (
        <ThumbsUp size={17} fill={liked ? "white" : "currentColor"} />
      )}
      <div>{count}</div>
    </button>
  );
